...
const char* STA_SSID = "YOUR_WIFI_2.4G";   // 改成你家 2.4GHz WiFi 名
...
#define C_PINK  0xFCB5      // 粉色(脸红/爱心)
#define C_STAR  0xFFE0      // 星星眼黄色
#define C_TEAR  0x5D9F      // 眼泪淡蓝
...
bool          started = false;   // 信息屏是否已被某个视图替换
unsigned long bootMs  = 0;       // 开机时间戳(用于自动切猫脸)
unsigned long lastCmdMs = 0;     // 最后一次收到指令的时间(闲置打瞌睡)
bool          sleeping  = false; // 是否已进入睡觉表情
bool          idleEyes  = false; // 当前是否为"普通眼"(才做眨眼/瞟的待机动画)
unsigned long nextBlinkMs = 0;   // 下次眨眼时间
...
// Cat dress-up: ears + whiskers + nose + mouth, drawn over the eye views.
// Positions are fixed (not affected by eye wiggle) so the face frame stays put.
void drawCatFeatures() {
  const int16_t lcx = eyeLX(0) + EYE_W / 2;   // left  eye centre x
  const int16_t rcx = eyeRX(0) + EYE_W / 2;   // right eye centre x

  // Ears — FIXED near the top of the screen (not tied to eye Y, so they
  // stay well clear of the eyes), apex tilted slightly outward
  const int16_t eb = 50, ea = 6;              // ear base / apex y
  tft.fillTriangle(lcx - 24, eb, lcx + 24, eb, lcx - 8, ea, C_BLACK);
  tft.fillTriangle(rcx - 24, eb, rcx + 24, eb, rcx + 8, ea, C_BLACK);
  tft.fillTriangle(lcx - 13, eb - 6, lcx + 11, eb - 6, lcx - 4, ea + 14, C_PINK);
  tft.fillTriangle(rcx - 11, eb - 6, rcx + 13, eb - 6, rcx + 4, ea + 14, C_PINK);

  // Nose (pink, downward triangle) + little mouth, below the eyes
  const int16_t nx = DISP_W / 2;
  const int16_t ny = eyeY() + EYE_H + 30;
  tft.fillTriangle(nx - 8, ny - 6, nx + 8, ny - 6, nx, ny + 5, C_PINK);
  tft.drawLine(nx, ny + 5, nx, ny + 11, C_BLACK);
  tft.drawLine(nx, ny + 11, nx - 8, ny + 16, C_BLACK);
  tft.drawLine(nx, ny + 11, nx + 8, ny + 16, C_BLACK);

  // Whiskers — 3 per side, WIDE fan from the cheeks out to the edges
  for (int8_t i = -1; i <= 1; i++) {
    tft.drawLine(nx - 22, ny + i * 7, 8,          ny + i * 26, C_BLACK);
    tft.drawLine(nx + 22, ny + i * 7, DISP_W - 8, ny + i * 26, C_BLACK);
  }
}
...
// 工作中表情:半眯的专注眼 + 下方思考的三个点 "..."
void drawWorking() {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0), rx = eyeRX(0), ey = eyeY();
  const int16_t h = EYE_H / 2;                 // 半高 = 眯眼专注的样子
  tft.fillRect(lx, ey, EYE_W, h, C_BLACK);
  tft.fillRect(rx, ey, EYE_W, h, C_BLACK);
  const int16_t cx = DISP_W / 2;
  const int16_t dy = ey + EYE_H + 18;
  for (int8_t i = -1; i <= 1; i++) tft.fillCircle(cx + i * 22, dy, 5, C_BLACK);
}
...
void drawSleepy() {
  ...
  tft.setTextSize(2); tft.setCursor(rx + EYE_W + 8,  cy - 14); tft.print("z");
  tft.setTextSize(3); tft.setCursor(rx + EYE_W + 22, cy - 46); tft.print("Z");
}
...
void drawSurprised() {
  ...
  tft.fillCircle(lcx, cy, 24, C_BLACK);
  tft.fillCircle(rcx, cy, 24, C_BLACK);
  tft.fillCircle(lcx, cy, 9, animBgColor);
  tft.fillCircle(rcx, cy, 9, animBgColor);
  tft.drawCircle(DISP_W / 2, cy + 62, 11, C_BLACK);
}

// 爱心眼
void drawHeart(int16_t cx, int16_t cy, int16_t s, uint16_t col) {
  tft.fillCircle(cx - s / 2, cy - s / 3, s * 6 / 10, col);
  tft.fillCircle(cx + s / 2, cy - s / 3, s * 6 / 10, col);
  tft.fillTriangle(cx - s, cy - s / 4, cx + s, cy - s / 4, cx, cy + s, col);
}
void drawHeartEyes() {
  tft.fillScreen(animBgColor);
  const int16_t lcx = eyeLX(0) + EYE_W / 2, rcx = eyeRX(0) + EYE_W / 2, cy = eyeCY();
  drawHeart(lcx, cy, 20, C_PINK);
  drawHeart(rcx, cy, 20, C_PINK);
}
...
void drawDizzy() {
  ...
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(lcx - 15, cy - 15 + t, lcx + 15, cy + 15 + t, C_BLACK);
    tft.drawLine(lcx + 15, cy - 15 + t, lcx - 15, cy + 15 + t, C_BLACK);
    tft.drawLine(rcx - 15, cy - 15 + t, rcx + 15, cy + 15 + t, C_BLACK);
    tft.drawLine(rcx + 15, cy - 15 + t, rcx - 15, cy + 15 + t, C_BLACK);
  }
}

// 弧形眼:d>0 笑眼(‿)
void arcEye(int16_t cx, int16_t cy, int16_t w, int16_t d, uint16_t col) {
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(cx - w, cy + t, cx,     cy + d + t, col);
    tft.drawLine(cx,     cy + d + t, cx + w, cy + t, col);
  }
}
// 四角星 ✦
void drawStar4(int16_t cx, int16_t cy, int16_t r, uint16_t col) {
  tft.fillTriangle(cx, cy - r, cx - r / 3, cy, cx + r / 3, cy, col);
  tft.fillTriangle(cx, cy + r, cx - r / 3, cy, cx + r / 3, cy, col);
  tft.fillTriangle(cx - r, cy, cx, cy - r / 3, cx, cy + r / 3, col);
  tft.fillTriangle(cx + r, cy, cx, cy - r / 3, cx, cy + r / 3, col);
}
...
void drawWink() {
  ...
  tft.fillRect(lx, ey, EYE_W, EYE_H, C_BLACK);
  arcEye(rx + EYE_W / 2, cy, EYE_W / 2, 12, C_BLACK);
}
// ☺️ 开心(笑弧眼)
void drawHappy() {
  tft.fillScreen(animBgColor);
  const int16_t lcx = eyeLX(0) + EYE_W / 2, rcx = eyeRX(0) + EYE_W / 2, cy = eyeCY();
  arcEye(lcx, cy, EYE_W / 2, 12, C_BLACK);
  arcEye(rcx, cy, EYE_W / 2, 12, C_BLACK);
}
// 😠 生气(怒眉)
void drawAngry() {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0), rx = eyeRX(0), ey = eyeY();
  tft.fillRect(lx, ey + 10, EYE_W, EYE_H - 10, C_BLACK);
  tft.fillRect(rx, ey + 10, EYE_W, EYE_H - 10, C_BLACK);
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(lx, ey - 6 + t, lx + EYE_W, ey + 6 + t, C_BLACK);
    tft.drawLine(rx, ey + 6 + t, rx + EYE_W, ey - 6 + t, C_BLACK);
  }
}
// 😢 难过(蓝眼泪)
void drawSad() {
  ...
  tft.fillRect(lx, ey, EYE_W, EYE_H, C_BLACK);
  tft.fillRect(rx, ey, EYE_W, EYE_H, C_BLACK);
  const int16_t tx = rx + EYE_W / 2, ty = ey + EYE_H + 4;
  tft.fillCircle(tx, ty + 12, 7, C_TEAR);
  tft.fillTriangle(tx - 7, ty + 12, tx + 7, ty + 12, tx, ty, C_TEAR);
}
// 🤩 星星眼
void drawStarEyes() {
  tft.fillScreen(animBgColor);
  const int16_t lcx = eyeLX(0) + EYE_W / 2, rcx = eyeRX(0) + EYE_W / 2, cy = eyeCY();
  drawStar4(lcx, cy, 22, C_STAR);
  drawStar4(rcx, cy, 22, C_STAR);
}
// 😊 害羞(脸红)
void drawBlush() {
  tft.fillScreen(animBgColor);
  ...
  arcEye(lcx, cy, EYE_W / 2, 12, C_BLACK);
  arcEye(rcx, cy, EYE_W / 2, 12, C_BLACK);
  tft.fillCircle(lcx - 6, cy + 28, 9, C_PINK);
  tft.fillCircle(rcx + 6, cy + 28, 9, C_PINK);
}
// 😎 墨镜
void drawCool() {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0), rx = eyeRX(0), cy = eyeCY();
  tft.fillRect(lx - 4, cy - 13, EYE_W + 8, 26, C_BLACK);
  tft.fillRect(rx - 4, cy - 13, EYE_W + 8, 26, C_BLACK);
  tft.fillRect(lx + EYE_W + 4, cy - 4, EYE_GAP - 8, 5, C_BLACK);
  arcEye(DISP_W / 2, cy + 46, 18, 9, C_BLACK);
}
...
void routeCmd() {
  started = true;   // 收到指令,取消开机自动切脸
  lastCmdMs = millis(); sleeping = false;   // 重置闲置计时
  ...
  idleEyes = false;
  switch (c) {
    case 'w': currentView = VIEW_EYES_NORMAL; idleEyes = true; nextBlinkMs = millis() + 2500; animNormalEyes(); break;
    ...
    case 'k':
      currentView = VIEW_EYES_NORMAL; drawWorking(); break;
    case 'z':
      currentView = VIEW_EYES_NORMAL; sleeping = true; drawSleepy(); break;
    case 'o':
      currentView = VIEW_EYES_NORMAL; drawSurprised(); break;
    case 'l':
      currentView = VIEW_EYES_NORMAL; drawHeartEyes(); break;
    case 'x':
      currentView = VIEW_EYES_NORMAL; drawDizzy(); break;
    case 'i': currentView = VIEW_EYES_NORMAL; drawWink();     break;
    case 'p': currentView = VIEW_EYES_NORMAL; drawHappy();    break;
    case 'g': currentView = VIEW_EYES_NORMAL; drawAngry();    break;
    case 'b': currentView = VIEW_EYES_NORMAL; drawSad();      break;
    case 'e': currentView = VIEW_EYES_NORMAL; drawStarEyes(); break;
    case 'h': currentView = VIEW_EYES_NORMAL; drawBlush();    break;
    case 'n': currentView = VIEW_EYES_NORMAL; drawCool();     break;
    case 'a':
...
void setup() {
  ...
  // ── Start WiFi ─────────────────────────────────────────────
  // Try to join home WiFi (STA). If it fails, fall back to AP so the
  // device is always reachable.
  WiFi.mode(WIFI_STA);
  WiFi.begin(STA_SSID, STA_PASS);
  unsigned long wifiT0 = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - wifiT0 < 15000) delay(250);

  bool staOK = (WiFi.status() == WL_CONNECTED);
  String netLine, ipLine, subLine;
  if (staOK) {
    MDNS.begin("clawd");              // 可用 http://clawd.local 访问
    netLine = String("WiFi: ") + STA_SSID;
    ipLine  = WiFi.localIP().toString();
    subLine = "or http://clawd.local";
  } else {
    WiFi.mode(WIFI_AP);
    WiFi.softAP(AP_SSID, AP_PASS);
    netLine = "AP: ClaWD-Mochi";
    ipLine  = "192.168.4.1";
    subLine = "password: clawd1234";
  }
  ...
  server.begin();
  bootMs = millis();      // 信息屏停留几秒后自动切到眼睛(见 loop)
  lastCmdMs = millis();
}
...
void loop() {
  server.handleClient();
  // 开机后约 4 秒、若还没收到任何指令,自动显示猫脸(普通圆眼)
  if (!started && millis() - bootMs > 4000) {
    started = true;
    lastCmdMs = millis();
    currentView = VIEW_EYES_NORMAL;
    idleEyes = true; nextBlinkMs = millis() + 2500;
    animNormalEyes();
  }
  // 活的待机:普通眼时定期眨眼,每第 4 次改成左右瞟一下
  if (started && idleEyes && !sleeping && millis() > nextBlinkMs) {
    blinkCount++;
    if (blinkCount % 4 == 0) {
      drawNormalEyes(7, false);  delay(380);
      drawNormalEyes(-7, false); delay(380);
      drawNormalEyes(0, false);
    } else {
  ...
  // 闲置 60 秒没收到任何指令 → 打瞌睡(z Z)
  if (started && !sleeping && !termMode && millis() - lastCmdMs > 60000) {
    sleeping = true;
    drawSleepy();
  }
}
